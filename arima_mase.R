...
for (idr in 1:nrow(nn5_dataset)) {
  ...
  for(index in 1 : (predicion_horizon-1) ){
    if(index ==1){
      ...
      ets_model_12 = auto.arima(ts(time_seres, frequency = 7))
      ...
      ets_model = auto.arima(ts(time_seres, frequency = 7))
...
write.table(file_write, file="sMAPE_arima.csv", row.names = F, col.names=c('sMAPE','MASE'), sep=",", quote=F)